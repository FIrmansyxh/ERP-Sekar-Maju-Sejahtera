...
|-------------|------|--------|------|-------|--------------------|
| Master Petani | `GET /petani` | `POST /petani` (ID dari FE dipakai bila kosong; ditolak = tidak dibuat lokal) | `PUT /petani/{id}` langsung, layar berubah setelah server menerima; verifikasi nama, HP, alamat, status | tidak ada (nonaktif = ubah) | Selesai: `PUT /petani/{id}/ganti-id` tersedia (cascade ke kupon dan bal) |
| Master Harga Beli | `GET /master/harga-beli` | `POST` (upsert) | `POST` (upsert) langsung, layar berubah setelah server menerima; verifikasi harga dan status | tidak ada di UI | Upsert harus menyimpan `status` dan tidak membuat baris ganda untuk `harga_id` yang sama |

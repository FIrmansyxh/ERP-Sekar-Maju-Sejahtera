...
# Replace the scan section
jsx_replacement = """              {/* 3-Step Scan Form */}
...
content = re.sub(r'\{/\* Input Box with Dropdown Autocomplete \*/\}.*?\{isBalDropdownOpen && scanSampleInput\.trim\(\)\.length > 0 && \(.*?\n\s*\}\n\s*\</div>\n\s*\)', jsx_replacement, content, flags=re.DOTALL)

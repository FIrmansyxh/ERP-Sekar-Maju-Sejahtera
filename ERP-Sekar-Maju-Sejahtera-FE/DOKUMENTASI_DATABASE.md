...
Tanggal: 2026-09-20, diperbarui 2026-09-21 (audit menyeluruh, lihat `LAPORAN_AUDIT_2026-09-21.md`) dan 2026-09-24 (server sebagai satu-satunya sumber data, lihat bagian 5.5 dan 10) · Berlaku untuk: frontend v3.0.x (repositori ini) dan backend Laravel + PostgreSQL (repositori terpisah).
...
| `POST /transaksi/{id}/bal` | tambah bal ke kupon (Sortir, bal susulan) | `items[]`; No. Bal yang sudah ada di kupon tidak digandakan (aman dikirim ulang); No. Bal di kupon lain / kupon lunas = 422 |
| `PATCH /transaksi/{id}/bal/{no_bal}` | ubah SATU bal (timbang, buka kunci, ganti tikar, ganti No. Bal / grade) | `{item_id?, perubahan:{...}, timbang_diubah_pada?, ganti_tikar_diubah_pada?}`; hanya kolom di `perubahan` yang ditulis; berat/GT dengan cap waktu lebih tua dari yang tersimpan diabaikan; bal tidak ada = 404; kupon lunas = 422; kupon terhapus = 410 |
| `DELETE /transaksi/{id}/bal/{no_bal}` | hapus SATU bal | bal yang sudah tidak ada = 200 (aman dikirim ulang); bal yang sudah jadi stok / kupon lunas = 422 |
| `PATCH /transaksi/{id}` | tutup sortir / ubah catatan | `{status_tahap?, catatan?, catatan_qc?}`; tahap menunggu timbang / lengkap dihitung server dari bal |
| `GET /sync/perubahan?sejak=` | sinkron lintas perangkat | Tanpa `sejak` = semua data (`penuh:true`). Dengan `sejak` (= `server_time` jawaban sebelumnya) = baris yang berubah (tumpang 30 detik) + `dihapus` per entitas. Lihat bagian 5.5 |
| `PUT /transaksi/{id}/sortir-items` | (lama) mengganti seluruh daftar bal | Tidak dipakai FE lagi sejak 2026-09-24; dipertahankan untuk tab versi lama selama masa transisi |
...
**Simpanan kupon sejak 2026-09-24** (dikerjakan `ErpApiService.kirimOperasiKupon`, dipanggil `antrianKupon`): setiap
tombol Sortir/Timbangan/Kasir menjadi SATU operasi: buka kupon = `POST sortir`, tambah bal = `POST .../bal`, timbang /
buka kunci / GT / edit bal = `PATCH .../bal/{no_bal}`, hapus bal = `DELETE .../bal/{no_bal}`, selesai sortir =
`PATCH /transaksi/{id}`, bayar = `PUT bayar`. Operasi satu kupon dikirim berurutan. Tidak ada lagi pengiriman seluruh
isi kupon dari layar (yang dulu bisa menghidupkan lagi bal yang sudah dihapus atau menimpa balik GT/berat).
...
### 5.5 Server sebagai satu-satunya sumber data (perbaikan 2026-09-24)

Keluhan yang tersisa setelah 5.4: tambah data tidak bertambah di komputer lain, edit tidak berganti, dan yang paling
sering **data yang dihapus muncul lagi**; intinya CRUD tidak seragam antar perangkat/akun.

**Akar masalah** (desain, bukan satu bug):

| Akar | Akibat |
|------|--------|
| Layar menyimpan dulu di komputer itu, lalu mengirim di belakang lewat antrean. Penolakan server (422, dll.) hanya dicoba ulang tiap 5 menit, sementara perubahan tetap tampil di komputer itu | Di komputer A data "sudah diedit/dihapus", di komputer B dan di database tidak. Setelah cache peramban dibersihkan / pindah komputer, data "kembali" |
| Kupon dikirim sebagai salinan UTUH dari layar lalu digabung dengan versi server memakai tebakan (penanda hapus per komputer, "berat lebih besar menang", dsb.) | Salinan basi di komputer lain menghidupkan lagi bal yang sudah dihapus dan menimpa balik GT/berat/grade |
| Kupon yang sudah lunas diedit lalu dikirim ke `sortir-items` yang menolaknya (422) | Edit hanya tampil di satu komputer dan tertunda selamanya |
| Login cadangan (server sempat lambat saat login) berjalan tanpa token; semua permintaan 401 dan daftar jatuh ke cache lokal diam-diam | Seluruh sesi bekerja pada data lokal: data baru tidak terlihat, simpanan tidak sampai |
| Data dari komputer lain hanya dimuat saat login, buka menu, tiap 10 detik di sebagian menu, dan tidak berkala di Sortir | Tambahan dari komputer lain lama sekali terlihat, atau tidak terlihat |
| Statistik petani dan tanda Surat Jalan pada batch/bal dihitung dan disimpan per komputer; tanda DO di batch dikirim sebagai permintaan terpisah | Angka laporan berbeda antar komputer; batch dan Surat Jalan tidak cocok bila salah satu kiriman gagal |

**Perbaikan (FE + BE):**

1. **Perubahan selain kupon langsung ke server** (`services/mutasiServer.ts`, `App.kirimKeServer`). Layar baru berubah
   setelah server menyimpan; bila ditolak / tidak terjangkau, alasannya ditampilkan dan data tetap. Form tetap terbuka
   agar bisa diulang. Tidak ada lagi antrean mutasi, overlay, dan penanda hapus per komputer.
2. **Kupon sebagai operasi per bal** (`services/operasiKupon.ts`, `services/antrianKupon.ts`, endpoint baru di bagian 5).
   Operasi langsung tampil di layar dan dikirim berurutan. Ditolak server (4xx) = dibatalkan saat itu juga dan
   dilaporkan; kupon terhapus (410) = dibuang; sesi habis (401) = minta login ulang; hanya gangguan jaringan yang
   ditahan dan dicoba ulang. Layar = kupon dari server + operasi yang belum terkirim.
3. **Sinkron lintas perangkat tiap 3 detik** (`GET /sync/perubahan`, `services/sinkronServer.ts`,
   `services/tokoDataServer.ts`, `hooks/useDataServer.ts`) untuk SEMUA menu termasuk Sortir dan laporan. Server
   menandai setiap perubahan dengan trigger (`updated_at` ikut berubah saat baris anak berubah, dan diisi jam database
   saat INSERT karena zona waktu Laravel dan PostgreSQL berbeda) dan mencatat setiap penghapusan di `sync_hapus`, jadi
   tidak ada jalur yang terlewat. Data yang lebih lama tidak pernah menimpa yang lebih baru; ID yang baru dihapus dari
   perangkat ini tidak bisa dihidupkan lagi oleh jawaban yang terlambat.
4. **Relasi dijaga server:** tanda "sudah dikirim DO" dan status batch dihitung ulang server setiap Surat Jalan dibuat,
   diubah, atau dibatalkan (`App\Support\RelasiBatchSample`); pelunasan divalidasi server (sortir ditutup, semua bal
   ditimbang, kirim ulang tidak menggandakan stok); No. Bal unik di seluruh gudang; baris kupon dikunci (`FOR UPDATE`)
   selama diubah.
5. **Sesi:** 401 dari server = keluar dan minta login ulang (antrean kupon dilanjutkan setelah login); login cadangan
   menghapus token akun sebelumnya; logout mencabut token di server; menonaktifkan akun mencabut semua tokennya.
6. **Laporan:** statistik petani dan tanda Surat Jalan pada bal diturunkan dari data server (`utils/turunanData.ts`);
   membuka laporan memuat ulang penuh dari server.
7. **Antrean versi lama dibersihkan sekali** (`utils/antreanLama.ts`): isinya perubahan yang tidak pernah diterima
   server; dibuang dan operator diberi tahu daftarnya.

**Bukti uji:** `tests/Feature/SinkronRelasiTest.php` (BE, PostgreSQL sungguhan) dan `scripts/uji-dua-perangkat/`
(dua proses "komputer" dengan penyimpanan terpisah menjalankan kode FE yang sama terhadap backend sungguhan; 57
pemeriksaan: tambah/edit/hapus lintas akun, bal dihapus tidak muncul lagi, perangkat offline, kupon dihapus saat
perangkat lain menyimpan, Sortir-Timbang-Bayar, batch & Surat Jalan, dan kesamaan data laporan).

**Langkah deploy (wajib bersamaan, backend dulu):** backend `git pull` → `php artisan erp:perbarui-skema` (menjalankan
`database/schema/schema.sql`, aman diulang; sudah ditambahkan ke `.github/workflows/deploy.yml`) → bersihkan cache
Laravel; lalu frontend. Bila frontend baru sempat berjalan terhadap backend lama: simpanan kupon ditahan dengan pesan
"Server belum diperbarui" (tidak dibuang) dan sinkron memakai muat ulang penuh berkala.
...
Berkas: `src/services/antrianKupon.ts`, `src/services/operasiKupon.ts`, `src/services/sinkronServer.ts`,
`src/services/tokoDataServer.ts`, `src/services/mutasiServer.ts`, `src/hooks/useDataServer.ts`, `src/App.tsx`,
`src/components/common/IndikatorSinkron.tsx`.

**Cara kerja sekarang (2026-09-24)**

```
Data di layar = data server
  login            → GET /sync/perubahan (semua data)
  tiap 3 detik     → GET /sync/perubahan?sejak=<server_time terakhir>   (tab terlihat; 30 detik bila di latar)
  fokus / online   → segera sinkron
  buka laporan     → muat ulang penuh

Perubahan selain kupon (petani, harga, pengguna, batch sample, Surat Jalan, hapus kupon)
  → langsung ke server → berhasil: layar dipasang dari jawaban server
                       → ditolak / tidak terjangkau: pesan tampil, layar tetap, form tetap terbuka

Kupon (Sortir, Timbangan, Kasir)
  → satu operasi per tombol (buat, tambah_bal, ubah_bal, hapus_bal, ubah_kupon, bayar)
  → langsung tampil (data server + operasi menunggu) → dikirim berurutan per kupon
       ├─ berhasil      → kupon dipasang dari jawaban server
       ├─ 4xx ditolak   → operasi dibatalkan + alasan ditampilkan (tidak disimpan diam-diam)
       ├─ 410 terhapus  → semua operasi kupon itu dibuang, kupon hilang dari layar
       ├─ 401/403       → keluar, minta login ulang; antrean lanjut setelah login
       └─ jaringan/5xx  → ditahan, dicoba ulang 2, 4, 8, 15, 30 detik lalu tiap 60 detik (juga setelah muat ulang)
```

**Yang terlihat operator:** lencana di Header hanya muncul bila perlu: "Menyimpan ke server (N)", kuning "N simpanan
belum sampai ke server" (tombol Kirim ulang), merah "Sesi login habis", dan kuning "Server tidak terhubung (data
terakhir HH:MM:SS)" bila layar sedang tidak mengikuti server. Pesan penolakan tampil di pojok kanan bawah.

**Diagnosa cepat**

1. Lihat Header: ada lencana? Arahkan kursor untuk rinciannya.
2. Konsol (F12): awalan `[Antrean kupon]`.
3. `localStorage['sms_antrian_kupon_v2']` = operasi kupon yang belum terkirim.
4. Server: `SELECT * FROM sync_hapus ORDER BY dihapus_pada DESC` = penghapusan terakhir yang disebarkan ke komputer lain.

**Batas yang perlu diketahui**

- Hanya operasi kupon yang boleh tertunda saat jaringan putus (supaya timbangan tidak berhenti). Perubahan lain
  memerlukan server tersambung.
- Operasi kupon yang tertunda di komputer A baru terlihat di komputer B setelah terkirim.
- Jejak audit masih per peramban (belum ada endpoint audit).
...
| `src/services/operasiKupon.ts` | Operasi kupon per bal dan penerapannya ke tampilan |
| `src/services/antrianKupon.ts` | Antrean operasi kupon: berurutan per kupon, penolakan, 410, login ulang, percobaan ulang |
| `src/services/sinkronServer.ts` | `GET /sync/perubahan` dan penggabungan paket ke daftar (`gabungDaftar`) |
| `src/services/tokoDataServer.ts` | Semua daftar data seperti di server (tanpa React), dipakai layar dan uji dua perangkat |
| `src/services/mutasiServer.ts` | Perubahan non-kupon langsung ke server |
| `src/hooks/useDataServer.ts` | Pembungkus React: sinkron berkala, cadangan localStorage |
| `src/utils/turunanData.ts` | Statistik petani dan penanda Surat Jalan pada bal, dihitung dari data server |
| `src/utils/kuponSortir.ts` | Hitung ulang kupon, aturan bal susulan |
...
(`sms_antrian_kupon_v2`, `sms_laporan_tampilan_*`). Antrean versi lama (`sms_antrian_sinkron_v1`, `sms_antrian_mutasi_v1`,
`sms_antrian_mutasi_terhapus_v1`, `sms_bal_dihapus_v1`) dibersihkan sekali saat versi baru dibuka. Kunci lama

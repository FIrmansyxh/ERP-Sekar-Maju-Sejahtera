import type { OperasiKupon } from '../services/operasiKupon';
...
export interface Petani {
  ...
  catatan?: string;
  /** Waktu terakhir baris ini berubah di server (updated_at). Data yang lebih lama tidak pernah menimpa yang lebih baru. */
...
export interface TransaksiItemBal {
  ...
  potongan_tara_kg?: number; // SB 2kg rata; HF: ≤49=3, 50–59=4, ≥60=5; TS & T: 30–49=4, 50–59=5, ≥60=6
...
export interface SaveTransaksiMeta {
  ...
  timpaPenuh?: boolean;
  /**
   * Perubahan yang dimaksud pemanggil (tambah / ubah / hapus satu bal, tutup sortir, bayar). Hanya ini yang dikirim
   * ke server, jadi bal lain di kupon yang sama tidak pernah tertimpa salinan layar yang basi. Tanpa ini, perubahan
   * diturunkan dari selisih kupon lama dan baru (turunkanOperasi di services/operasiKupon.ts).
   */
  operasi?: OperasiKupon[];
}

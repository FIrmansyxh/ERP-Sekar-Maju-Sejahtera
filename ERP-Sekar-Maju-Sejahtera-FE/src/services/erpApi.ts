/**
 * Panggilan API ERP Sekar Maju Sejahtera dan pemetaan jawaban server ke model layar.
 *
 * Server adalah satu-satunya sumber data. Daftar dimuat lewat sinkron lintas perangkat (sinkronServer.ts), perubahan
 * kupon dikirim sebagai operasi per bal (antrianKupon.ts), dan perubahan lain dikirim langsung ke server lalu layar
 * mengikuti jawabannya (mutasiServer.ts). Penyimpanan lokal hanya cadangan tampilan saat server tidak terjangkau.
 */
...
import type { OperasiKupon } from './operasiKupon';
import { 
  Petani, 
  Barang, 
  TransaksiPembelian, 
...
import { pulihkanStatusSampleLama, sortTransaksiItemsByInputOrder } from '../utils/kuponSortir';
...
/** updated_at dari server (ISO), dipakai supaya data lama tidak pernah menimpa data yang lebih baru di layar. */
export function waktuServer(raw: any): string | undefined {
  const v = raw?.updated_at ?? raw?.diperbarui_pada;
  return typeof v === 'string' && v ? v : undefined;
}

/** Isi satu bal untuk dikirim ke server (buat kupon / tambah bal). */
function payloadBal(it: Partial<TransaksiItemBal>) {
  return {
    item_id: it.item_id || null,
    no_bal: it.no_bal,
    kode_bal_pembeli: it.kode_bal_pembeli || null,
    barcode: it.barcode || it.no_bal || null,
...
/** ID bal buatan server ("TRX-...-BAL-01"); ID sementara dari layar tidak dikenal server. */
const idBalServer = (itemId?: string): string | null => (/-BAL-d+$/i.test(String(itemId || '')) ? String(itemId) : null);
...
export class ErpApiService {
  ...
  public static async login(username: string, password: string): Promise<{ success: boolean; user?: User; message: string; mode: 'api' | 'local' }> {
    ...
    try {
      ...
      if (isOnline) {
        ...
        if (res.status === 'success' && res.data) {
          ...
          // Akun ini dicatat di peramban (dengan hash sandi) supaya sesi bisa dipulihkan saat halaman dimuat ulang dan
          // login cadangan tetap bisa dipakai bila server kelak tidak terjangkau
          const hash = await hashPassword(password).catch(() => undefined);
          saveUserData([
            { ...localFormattedUser, ...(hash ? { password: hash } : {}) },
            ...loadUserData().filter((u) => u.user_id !== localFormattedUser.user_id),
          ]);
    ...
    // 2. Fallback autentikasi lokal. Token akun sebelumnya (mungkin akun orang lain) tidak boleh ikut terpakai:
    //    begitu server bisa dihubungi, sesi ini diminta login ulang ke server (lihat EVENT_SESI_HABIS).
    setAuthToken(null);
  ...
  public static async storeSortirTransaksi(tx: TransaksiPembelian): Promise<TransaksiPembelian> {
    ...
    const payload = {
      ...
      catatan: tx.catatan || '',
      items: itemKupon.map(payloadBal),
    };
  ...
  // --- OPERASI KUPON (dipakai antrianKupon.ts) ---
  /**
   * Mengirim SATU operasi kupon ke server dan mengembalikan kupon terbaru dari server. Melempar ApiError bila gagal
   * (status 0 = server tidak terjangkau, 4xx = ditolak, 410 = kupon sudah dihapus), supaya antrean bisa memutuskan:
   * coba ulang, batalkan dengan pesan, atau buang kupon dari layar.
   */
  public static async kirimOperasiKupon(transaksiId: string, op: OperasiKupon): Promise<TransaksiPembelian> {
    const id = encodeURIComponent(transaksiId);
    const jawab = (res: { status?: string; data?: unknown; message?: string }, pesan: string): TransaksiPembelian => {
      if (res.status === 'success' && res.data) return this.mapBackendTransaksi(res.data);
      throw new ApiError(res.message || pesan, 422);
    };
    switch (op.jenis) {
      case 'buat':
        return this.storeSortirTransaksi({ ...op.kupon, transaksi_id: transaksiId });
      case 'tambah_bal':
        return jawab(await api.post<any>(`/transaksi/${id}/bal`, { items: op.bal.map(payloadBal) }), 'Server menolak menambah bal');
      case 'ubah_bal':
        return jawab(
          await api.patch<any>(`/transaksi/${id}/bal/${encodeURIComponent(op.ref.no_bal)}`, {
            item_id: idBalServer(op.ref.item_id),
            perubahan: op.perubahan,
            ganti_tikar_diubah_pada: op.gt_diubah_pada ?? null,
            timbang_diubah_pada: op.timbang_diubah_pada ?? null,
          }),
          'Server menolak mengubah bal'
        );
      case 'hapus_bal': {
        const itemId = idBalServer(op.ref.item_id);
        const query = itemId ? `?item_id=${encodeURIComponent(itemId)}` : '';
        return jawab(await api.delete<any>(`/transaksi/${id}/bal/${encodeURIComponent(op.ref.no_bal)}${query}`), 'Server menolak menghapus bal');
      }
      case 'ubah_kupon':
        return jawab(await api.patch<any>(`/transaksi/${id}`, op.perubahan), 'Server menolak mengubah kupon');
      case 'bayar':
        return this.bayarTransaksi(transaksiId, op.metode, 'PMK-01', op.catatan_kasir);
    }
  }

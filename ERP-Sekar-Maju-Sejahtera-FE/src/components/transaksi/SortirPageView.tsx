...
import { refDariBal } from '../../services/operasiKupon';
...
interface SortirPageViewProps {
  ...
  /** false = server menolak / tidak terjangkau: kupon tetap ada */
  onDeleteTransaksi?: (transaksiId: string, alasan?: string) => void | boolean | Promise<void | boolean>;
...
export const SortirPageView: React.FC<SortirPageViewProps> = ({
  ...
  // Kupon yang dibuka sudah tidak ada di server (dihapus di komputer lain, atau pembuatannya ditolak server) dan tidak
  // ada simpanan yang masih menunggu: lepaskan dari layar supaya operator bisa membuka kupon baru.
  useEffect(() => {
    if (!openTxId || openTx || transaksiList.some((t) => t.transaksi_id === openTxId)) return;
    if (idKuponTerkini(openTxId) !== openTxId || antrianKupon.adaTugas(openTxId)) return;
    if (!statusServer.ambil().terakhirBerhasil) return;
    resetOpenTxId();
    resetSusulanMode();
    setScanFeedback({ text: 'Kupon yang dibuka sudah tidak ada di server (dihapus di komputer lain atau ditolak). Silakan buka kupon baru.', isError: true });
  ...
  const handleAddBalItem = async () => {
    ...
    // Kupon baru dibuat bersama bal pertamanya; kupon yang sudah ada hanya menerima bal ini (bal lain tidak ditulis ulang)
    const operasi: SaveTransaksiMeta['operasi'] = openTx ? [{ jenis: 'tambah_bal', bal: [itemBaru] }] : [{ jenis: 'buat', kupon: updatedTx }];
    onSaveTransaksi(
      updatedTx,
  ...
  const handleSaveEdit = (item: TransaksiItemBal) => {
    ...
    const beratIkutBerubah = (item.berat_kg || 0) > 0;
    onSaveTransaksi(
      updatedTx,
        ...
        operasi: [
          {
            ...
            ref: refDariBal(item),
            perubahan: {
              no_bal: updatedItem.no_bal,
              barcode: updatedItem.barcode,
              kode_grade: updatedItem.kode_grade,
              harga_per_kg: updatedItem.harga_per_kg,
              potongan_tara_kg: updatedItem.potongan_tara_kg,
              ...(beratIkutBerubah ? { berat_kg: updatedItem.berat_kg } : {}),
            },
            timbang_diubah_pada: updatedItem.diubah_lokal_pada,
          },
        ],
        audit: {
  ...
  const handleRemoveItem = async (itemId: string) => {
    ...
    // Hanya bal ini yang dihapus di server; perangkat lain tidak bisa menghidupkannya lagi dengan salinan lama
    onSaveTransaksi(updatedTx, [], {
      operasi: [{ jenis: 'hapus_bal', ref: refDariBal(item) }],
      audit: {
  ...
  const handleSelesaiSortir = () => {
    ...
    onSaveTransaksi(updatedTx, [], {
      operasi: [{ jenis: 'ubah_kupon', perubahan: { status_tahap: 'menunggu_timbang', catatan_qc: updatedTx.catatan_qc } }],
      audit: {
        aksi: 'SELESAI_SORTIR',
  ...
  const handleBatalkanKupon = async () => {
    ...
    if (!setuju) return;
    const terhapus = await onDeleteTransaksi(openTx.transaksi_id, 'Kupon dibatalkan dari Sortir sebelum selesai');
    if (terhapus === false) {
      setScanFeedback({ text: `Kupon ${openTx.no_kupon} belum dibatalkan. Periksa pesan di pojok layar lalu coba lagi.`, isError: true });
      return;
    }
    resetOpenTxId();

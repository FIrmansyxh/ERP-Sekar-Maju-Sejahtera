...
export const TimbanganPageView: React.FC<TimbanganPageViewProps> = ({
  ...
  const handleToggleGantiTikar = (itemId: string) => {
    ...
    const gtBaru = nextItems.find((it) => it.item_id === itemId);
    // Hanya centang GT bal ini yang dikirim; bal lain di kupon tidak ikut tertulis
    onSaveTransaksi(updatedTx, [], {
      skipAudit: true,
      operasi: [
        {
          jenis: 'ubah_bal',
          ref: refDariBal(target),
          perubahan: { ganti_tikar: nextGanti, potongan_tikar: gtBaru?.potongan_tikar || 0 },
          gt_diubah_pada: gtBaru?.gt_diubah_pada || Date.now(),
        },
      ],
    });
    setScanFeedback({
  ...
  const handleApplyWeightForActiveBal = () => {
    ...
    // Tarif tikar yang diubah di form timbang ikut dikirim (sebagai perubahan GT bertanda waktu)
    const tarifTikarBerubah = Boolean(activeBalItem.ganti_tikar) && livePotTikar !== (activeBalItem.potongan_tikar || 0);
    onSaveTransaksi(
      updatedTx,
        ...
        operasi: [
          {
            ...
            perubahan: {
              ...
              potongan_kuli: livePotKuli,
              potongan_tali: livePotTali,
              ...(tarifTikarBerubah ? { ganti_tikar: true, potongan_tikar: livePotTikar } : {}),
            },
            timbang_diubah_pada: weighedItem.diubah_lokal_pada || Date.now(),
            ...(tarifTikarBerubah ? { gt_diubah_pada: Date.now() } : {}),
          },
        ],
      }
    );
          ...
          operasi: [
            {
              jenis: 'ubah_bal',
              ref: refDariBal(item),
              perubahan: { berat_kg: 0, berat_bruto_kg: existingBruto },
              timbang_diubah_pada: unlockedItem.diubah_lokal_pada || Date.now(),
            },
          ],
        }
      );

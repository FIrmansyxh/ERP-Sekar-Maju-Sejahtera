...
interface PengirimanManagementProps {
  ...
  userRole: UserRole;
  /** Selesai setelah server menjawab; false = tidak tersimpan (form tetap terisi agar bisa diulang). */
  onSaveNewPengiriman: (pengiriman: PengirimanBarang, updatedBarangIds: string[]) => boolean | void | Promise<boolean | void>;
  /** Menyimpan perubahan Surat Jalan yang belum Selesai; mengembalikan false bila ditolak (mis. bal dipakai Surat Jalan lain). */
  onUpdatePengiriman?: (pengiriman: PengirimanBarang, balDitambah: string[], balDikeluarkan: string[]) => boolean | Promise<boolean>;
...
export const PengirimanManagement: React.FC<PengirimanManagementProps> = ({
  ...
  const handleConfirmSave = async () => {
    ...
    if (existingPengiriman) {
      ...
      const berhasil = await Promise.resolve(onUpdatePengiriman(newPengiriman, balDitambah, balDikeluarkan)).finally(() => setSedangMenyimpan(false));
      if (!berhasil) {
      ...
      setSedangMenyimpan(true);
      const berhasil = await Promise.resolve(onSaveNewPengiriman(newPengiriman, selectedBalIds)).finally(() => setSedangMenyimpan(false));
      if (berhasil === false) {
        setIsConfirmOpen(false);
        return;
      }

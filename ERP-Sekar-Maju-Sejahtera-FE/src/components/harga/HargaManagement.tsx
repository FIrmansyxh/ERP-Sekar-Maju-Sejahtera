...
interface HargaManagementProps {
  ...
  onSaveNewPrice: (newPrice: TabelHarga, oldPriceIdToArchive?: string) => void | boolean | Promise<void | boolean>;
...
export const HargaManagement: React.FC<HargaManagementProps> = ({
  ...
  const handleSaveForm = async (e: React.FormEvent) => {
    ...
    const tersimpan = await onSaveNewPrice(newItem, existingActiveGrade ? existingActiveGrade.harga_id : (editingItem ? editingItem.harga_id : undefined));
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            {(searchTerm.trim() || statusFilter !== 'all') && (
              <span className="text-[11px] text-gray-500 font-medium">
                Ditemukan: <strong className="text-gray-900">{filteredList.length}</strong> grade
              </span>
            )}
          </div>
          ...
          <div className="w-full sm:w-80 md:w-96">
            <div className="relative">
              ...
              {searchTerm && (
                <button
                  type="button"
                  id="btn-clear-search-harga-beli"
                  onClick={() => {
                    setSearchTerm('');
                    setCurrentPage(1);
                  }}
              ...
              <div>
                ...
                {formHarga !== '' && Number(formHarga) > 0 && (
                  <p className="text-[11px] text-gray-600 mt-1">
                    Terbaca: <strong className="text-gray-900">{formatRupiah(Number(formHarga))}</strong> per kilogram
                  </p>
                )}
              </div>

...
interface SampleManagementProps {
  ...
  userRole: UserRole;
  /** false = server menolak / tidak terjangkau: isian tetap ada agar bisa diulang */
  onSaveBatchSample: (newBatch: BatchPengirimanSample, updatedBarangs: Barang[]) => void | boolean | Promise<void | boolean>;
...
export const SampleManagement: React.FC<SampleManagementProps> = ({
  ...
  const handleSaveBatchForm = async (sebagai: 'draft' | 'final' = 'final') => {
    ...
    const tersimpan = sedangEdit ? await onUpdateBatchSample(newBatch, updatedBarangs) : await onSaveBatchSample(newBatch, updatedBarangs);
    setIsConfirmCreateOpen(false);

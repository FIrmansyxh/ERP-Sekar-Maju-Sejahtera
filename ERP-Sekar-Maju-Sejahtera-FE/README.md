...
melalui REST API. **Server adalah satu-satunya sumber data**; `localStorage` hanya cadangan tampilan.

- **Layar mengikuti server.** Saat login semua data dimuat, lalu tiap 3 detik (selama tab terlihat) aplikasi mengambil
  perubahan sejak pertanyaan terakhir (`GET {API}/sync/perubahan`). Tambah, ubah, dan hapus dari komputer atau akun
  lain terlihat dalam beberapa detik di semua menu, termasuk Sortir dan laporan.
- **Server tidak tersedia:** aplikasi menampilkan data terakhir yang diketahui dan Header menampilkan "Server tidak
  terhubung". Login cadangan memakai akun yang tersimpan di peramban; begitu server bisa dihubungi, pengguna diminta
...
- **Perubahan selain kupon langsung ke server** (petani, harga, pengguna, batch sample, Surat Jalan, hapus kupon).
  Layar baru berubah setelah server menyimpan. Bila server menolak atau tidak terjangkau, alasannya ditampilkan dan
  data tidak berubah di komputer mana pun, sehingga tidak ada lagi "sudah diedit tapi di komputer lain tidak".
- **Kupon (Sortir, Timbangan, Kasir) dikirim sebagai operasi per bal** (tambah bal, timbang, ganti tikar, hapus bal,
  tutup sortir, bayar). Operasi langsung tampil lalu dikirim berurutan; server menerapkannya ke data terbarunya,
  jadi salinan layar yang basi tidak bisa menimpa bal lain atau menghidupkan lagi bal yang sudah dihapus. Operasi
  yang ditolak server dibatalkan saat itu juga dengan pesan; hanya gangguan jaringan yang ditahan dan dicoba ulang
  (Header: "N simpanan belum sampai ke server").
- Rincian desain, kontrak API, dan bukti uji: `DOKUMENTASI_DATABASE.md` bagian 5.5 dan 10. Jejak audit masih per
  peramban.
...
Uji dua perangkat terhadap backend sungguhan (database UJI, bukan produksi) menjalankan kode aplikasi yang sama di dua
proses dengan penyimpanan terpisah dan memeriksa 57 skenario lintas komputer/akun (tambah, edit, hapus, offline,
Sortir-Timbang-Bayar, batch & Surat Jalan, kesamaan data laporan):
...
│   ├── sinkronServer.ts     Ambil perubahan dari server (/sync/perubahan) dan gabungkan ke daftar di layar
│   ├── tokoDataServer.ts    Semua daftar data seperti di server (tanpa React)
│   ├── operasiKupon.ts      Operasi kupon per bal (tambah, timbang, GT, hapus, tutup sortir, bayar)
│   ├── antrianKupon.ts      Kirim operasi kupon berurutan; ditolak = batal + pesan, jaringan putus = coba ulang
│   ├── mutasiServer.ts      Perubahan selain kupon, langsung ke server
│   └── statusServer.ts      Status hubungan ke server untuk Header
├── components/
│   ├── auth/                Halaman login

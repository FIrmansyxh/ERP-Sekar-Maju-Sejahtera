...
content = content.replace("  const [showStatusFilter, setShowStatusFilter] = useState<FilterStatusOption>('all');", "  const [showStatusFilter, setShowStatusFilter] = useState<FilterStatusOption>('all');\n" + state_code)

with open('src/components/pengiriman/StatusBatchPengirimanManagement.tsx', 'w') as f:
    f.write(content)
print("fixed status batch lint 2")

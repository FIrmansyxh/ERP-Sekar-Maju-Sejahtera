...
# We need to find the place where `{isActiveBalWeighed && (` banner ends and inject the form back.
# Looking at the current file, it ends with:
#                 )}
#                 
# 
...
# Replace in content where the empty space is
target_pattern = r"(                \)\}\s*)"
# Wait, let's just use string replace for the exact spot where we know it's missing.
# We know `(Mode Baca).</p>\n                    </div>\n                  </div>\n                )}\n` is where the banner ends.

find_str = """                  </div>\n                )}\n                \n\n"""
# Actually it might not be exactly that whitespace.
content = re.sub(r"(Mode Baca\)\.</p>\s*</div>\s*</div>\s*)\}\s*", r"\1}\n" + form_code + r"\n          ) : (", content)

# But wait, looking at my cat earlier, the file currently has:
#                 {isActiveBalWeighed && (
# ...
#                 )}
#
# (empty space)
# (End of file / some other code)
#
# Let's inspect line 935 to 945

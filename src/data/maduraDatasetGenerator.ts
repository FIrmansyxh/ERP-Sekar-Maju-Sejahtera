...
export function generateMaduraTobaccoDataset(): GeneratorResult {
  ...
  const GRADE_PRICES: Record<string, number> = {};
    let hargaData = INITIAL_HARGA_DATA;
  try {
    if (typeof window === 'undefined') throw new Error('SSR');
  ...
  while (currentBal <= TOTAL_BALS) {
    ...
    for (let b = 0; b < balCount; b++) {
      ...
      const assignedLocation = WAREHOUSE_LOCATIONS[currentBal % WAREHOUSE_LOCATIONS.length];
      const barang: Barang = {
        ...
        lokasi_gudang: assignedLocation,
    ...
    transaksiList.push({
      ...
      kode_grade: Array.from(new Set(trxItems.map(it => it.kode_grade))).sort().join(', '),
      lokasi_gudang: WAREHOUSE_LOCATIONS[trxCounter % WAREHOUSE_LOCATIONS.length],

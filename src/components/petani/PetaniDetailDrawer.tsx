...
export const PetaniDetailDrawer: React.FC<PetaniDetailDrawerProps> = ({
  ...
  const totalBalComputed = realTransactions.reduce((acc, tx) => acc + (tx.total_bal || (tx.items ? tx.items.length : 0)), 0);
    ...
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex justify-end font-sans">
      <div className="bg-white w-full max-w-xl h-full shadow-2xl flex flex-col border-l border-gray-300 text-xs text-gray-800 animate-in slide-in-from-right duration-200">
        ...
        <div className="flex border-b border-gray-200 bg-[#f8f9fa] px-4 pt-2">
          ...
          >
            Riwayat Setoran ({realTransactions.length} Kupon)
          </button>
          <button
          ...
          {activeTab === 'transaksi' && (
            <div className="space-y-3">
              <div className="text-xs text-gray-600">
                Daftar transaksi setoran yang telah disetor oleh petani:
              </div>
